...
During this course we really invite you to seek interaction on the course contents, the exercises and assignments with fellow course participants and the instructors. To fascilitate collaboration and also the possibility for you to build a strong "class of `r lubridate::year(Sys.Date())`" network, we scheduled 5 hour sessions at the HU. We will of course include breaks. Your teachers may not always be present during the whole session, but quite frequently we will. We will start every class with a plenary meeting.
