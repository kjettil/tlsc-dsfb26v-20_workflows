...
author: "Alyanne de Haan, PhD; Bas van Gestel; Rene van der Ploeg, PhD; Chris van Oevelen, PhD; Marc Teunis, PhD"
date: "`r Sys.time()`"
...
During the course you can still work on the server you used in DAUR1. The Cloud Computing Environment provides web access to R and RStudio via a Virtual Machine. This machine runs a server edition of RStudio and has the latest R version and all the required packages available. However, you will also often use your own local Rstudio environment and other software (see chapter prerequisites!). You should be comfortable with working and fixing issues with R by now, enough to keep your local environment up and running.

<!-- In order to access the server you will need credentials, which you will recieve before the course starts. This is a convenient way to use R in a course and during the course we will only be using this environment. This is to ensure reproducibility and prevents a lot of trouble shooting from your side and the teacher's side. -->
