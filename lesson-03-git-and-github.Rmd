...
Subsequently, clone the GitHub repository to your computer and add your portfolio-Rmd-files. Commit/push the changes.

(B)
Now start building your CV in a new Rmarkdown file and keep using github for version control. Remember to commit often, and push your changes!

You can look around online for examples of cv's (/resume). You can use Jekyll if you like it and are up to a challenge  [click](https://workwithcarolyn.com/blog/digital-cv-guide) but it does make rendering a little more difficult. There are also [packages](https://cran.r-project.org/web/packages/vitae/vignettes/vitae.html) to build cv's in Rmarkdown (of course there are). Or google "create resume rmarkdown". But diving into these is completely optional. You already know how to use Rmarkdown, which is enough to build a cv. Your cv will be part of your portfolio.
