...
```{r, fig.cap = "You are here! But not in 2021, in 2021 we are at home"}
library(leaflet)
...
Change the markers in the example above to little icons of rabbits! See [here](http://rstudio.github.io/leaflet/markers.html) how to do this.
</div>
...
```{r, fig.cap="But with 3 axes you may want to be able to rotate the graph."}
library(threejs)
...
Try to reproduce the figure [here](http://bwlewis.github.io/1000_genomes_examples/PCA.html)

Start by downloading data from the [International Genome Sample Resource](https://www.internationalgenome.org/), we will use the same dataset as in the example: 
