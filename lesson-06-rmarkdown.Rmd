...
This workflow fits in a 'continuous integration' workflow. You can continue working on the same project with multiple people, and frequently integrate your work to see if there are any errors or you are accidentally working on the same part of the project as a coworker.

For example, when making this reader, Bas, Marc and Alyanne work within their own branches. Usually, we make a branch per task we take on for the day. We make sure we branch of from the main, and at least once a day we merge our work back into the main branch (Continuous integration). Usually this is at the end of the day. Two people may have worked on the same .Rmd file, but this is fine. Git merges them for us. We then make sure the main branch still knits to the html in this reader without crashing or otherwise messing up (Continuous testing). If we think the rest of the team would like to see an update, we publish the newest version of the reader to RSconnect, which publishes this reader as a website (Continuous deployment).
...
### Some basic advice and good practice
  ...
  - Do everything within your .Rmd file. Don't export data, change some numbers in Excel en load them back in. This is breaking the reproducable workflow. If you find a mistake in the raw data, fix it within the .Rmd and provide a comment explaining what you are doing.
...
Roughly, your average deliverable will consist of text, figures and tables. Text is pretty easy in RMarkdown.
...
Most tricks for making nice publication-worthy graphs themselves will be covered in a masterclass. And adding basic graphs to an RMarkdown is something you already learned in DSFB1.
...
```{r figdiam, fig.cap = "Cut and price of diamonds", echo=FALSE, fig.height = 5, fig.width = 7, fig.align = "center", }
#install.packages("ggridges")
#install.packages("viridis")
#install.packages("ggthemes")
library(viridis)
...
</mark> Waarom klopt figure number niet? </mark>
...
```{r cbpgraph1, fig.cap="colourblind penguin masses by species"}
...
\normalsize
```
For HTML, you will want to use CSS formatting.
...
```{rexamplecars, fig.cap ="We made this graph smaller", fig.width = 4, fig.height = 4, echo=FALSE}
ggplot(mpg, aes(displ, hwy, color = class)) + 
  geom_point() 
```
...
Here we use bg-danger for the code, and bg-warning for the output:
...
```
````
<mark> XXX This is not working. It should be. fix later. Vanwege externe style sheet in use misschien? </mark>
...
But defining all your styling stuff within your .Rmd gets messy quite quickly. So you can put them in a separate .css file. Put this file in the same folder as your .Rmd
To link one or multiple custom stylesheets to an Rmd document, you can use the css option in your YAML header:
...
Include a link to the wikipedia page of your topic. Make sure the link changes to a pink colour [when you hover over it](https://www.w3schools.com/css/css_link.asp). 
...
[^bignote]: Here's one with multiple paragraphs and code.
    ...
    Even code chunks work fine: `r seq(1:5)`
    
    And images:
    ![You found me!](images/korok.jpg){width=20%}
...
<mark> I should switch to Zotero now that Mendeley is owned by Elsevier :/ I will update when I have time. </mark>
...
##### Exercise `r les` {-}
Import some random paper from Pubmed into Mendeley (drag & drop the pdf into Mendeley). What metadata do you get? 

Import some pdf you rendered yourself from RMarkdown into Mendeley. What metadata do you get? 

Try making some notes and highlighting some text within Mendeley.
...
<mark> XXX screenshots be here </mark>

  3. Go back to your .Rmd file and change the YAML header: write ‘bibliography: library.bib’ (or whatever is the name of your .bib file), so your header will look like something this:
...
```
  ...
  4. Go back to Mendeley, and check the citation key of the random paper we downloaded. For example, suppose it is `@Author2000`

<mark> XXX screenshots be here </mark>

You can now cite papers it in your .Rmd using their citation key:
  
  ```
  Lisa said something really smart [@Author2000].
  ```
or two people:
...
<mark>XXX uitwerken: is dit genoeg?</mark>

  A) Make a new folder within Mendeley for the projecticum. Place all pdf's you used till now in your folder (if you did not use any papers as source yet, find 3 papers on Pubmed now that you could use for your introduction.). Copy your .bib file in the folder on your computer you are using for the Projecticum git repo.
  B) Start a .Rmd named "introduction.Rmd" with a few lines introduction on your projecticum topic.
  C) Include automatic inline references to the papers you used.
  D) The bibliography will be placed at the end of the document. Provide an appropriate header.
  E) Merge your work with that of your projecticum partner through github. Solve all the problems you encounter. If github does not want to merge .bib files and you want a merged one, try [here](https://caltechlibrary.github.io/bibtex/webapp/bibmerge.html).
