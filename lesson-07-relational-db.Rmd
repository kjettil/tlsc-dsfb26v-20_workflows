...
Run the installer with all the default settings. Remember your password. {#installposgreSQL}
...
You can also use ALTER TABLE to change the table name, but this works a bit weird. The schema needs to be specified in the old name, but not the new:
...
Or if we for instance want to add anything to the text in a column:
...
Go back to DBeaver and refresh the connection, and out pagila database will be filled!
...
```{r,  echo=FALSE, out.width = "500px", fig.cap="Making sure we are working in the correct database"}
knitr::include_graphics(
  here::here(
    "images",
    "DBeaver_pagila4.PNG"
...
This database contains fictional business processes of a DVD rental store. It contains 15 tables. We will use is to try some queries (a query is a request for data results from a database). In fact, we already did some queries, such as:
...
The following example uses the SELECT statement to return full names and emails of all customers, using `||` as the concatenating operator that concatenates one or more strings into a single string, and using `AS` to give the new column a descriptive name. Try what happens if you omit the "AS full_name".
...
## I really love this and want more
...
The query bit in this lesson borrows heavily from [postgresqltutorial.com/](https://www.postgresqltutorial.com/)
