...
We will focus on R Packages this lesson.
...
During the exercises it is important that youn perform all the steps in RStudio, not just read trhough them, but actually run all the code and perform the asctivities in the demo.

## Package structure
The most basic package consists of just one elementary file called the `DESCRPTION` file. This is the file that hold valuable information about:

 - Package name
 - Package version
 - Package authors
 - What the package does in more details
 - What are functions or packages are needed for the package (these are called dependencies)
 - What system dependencies (for example software in Linux, Mac or Windows) is needed additionally

To learn more about how R packages, we will look at such a DESCRIPTION file during an exercise:
...
To accomplish this: 
 ...
 1. Initiate an R package by running `usethis::create_package("./foobar")` from the R Console. R will ask you to overwrite the `foofactors.Rproj` file, you should choose 'yes'
...
<div class="question">
##### Exercise `r les` {-} 
After creating your commits and pushing to Github, try installing your own `{foofactors}` R package, using the function `devtools::install_github(<package_name/github_account_name>)` 
</div>
...
**To pass this assignment, your new package should at least include:**
 ...
 1. A properly formatted DESCRIPTION file, with the fields including relevant information
 ...
 1. The package installs from Github without errors when I run: `devtools::install_github("package_name/account_name")`
